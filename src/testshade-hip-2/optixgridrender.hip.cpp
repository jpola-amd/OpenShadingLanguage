...
OptixGridRenderer::synch_attributes()
{
    ...
    ustring userdata_str1("ud_str_1 %d %d %f");
...
OptixGridRenderer::make_optix_materials()
{
    ...
    // Renderer
    std::string program_ptx;
    program_ptx = load_ptx_file("optix_grid_renderer.bc");
    if (program_ptx.empty()) {
        errhandler().severefmt("Could not find BC for the renderer program");
        return false;
    }
    ...
    */
    ...
    //Error: the unnamed_group_1 says it is generated by the llvm19 but the other stuff as llvm 19 git  */
    /*
    ~/projects/hip-osl/3rdParty/llvm-artifacts-release-rocm-19/bin/llvm-link unnamed_group_1 -o x.bc
error: Invalid record (Producer: 'LLVM19.0.0git' Reader: 'LLVM 19.0.0git')

    same as: 
    /opt/rocm/llvm/bin/llvm-link ./unnamed_group_1 -o x.bc
    error: Invalid record (Producer: 'LLVM19.0.0git' Reader: 'LLVM 19.0.0git')

    llvm-dis is not working as well:
    ~/projects/hip-osl/3rdParty/llvm-artifacts-release-rocm-19/bin/llvm-dis unnamed_group_1 -o dis.ll
/home/jpola/projects/hip-osl/3rdParty/llvm-artifacts-release-rocm-19/bin/llvm-dis: error: Invalid record (Producer: 'LLVM19.0.0git' Reader: 'LLVM 19.0.0git')

    this means that the unnamed_group_1 is corrupted somehow or just the generated tag is incorrect.
    How to generate the proper tag:<unnamed>
    1. check the liboslexec version and what it is linking
    ...
    */
    ...
    std::vector<uint8_t> hip_fatbin; 
    {
        ...
        HIPRTC_CHECK(hiprtcLinkAddData(linkState, HIPRTC_JIT_INPUT_LLVM_BITCODE, program_ptx.data(), program_ptx.size(), "optix_grid_renderer.bc", 0, nullptr, nullptr));
...
OptixGridRenderer::render(int xres OSL_MAYBE_UNUSED, int yres OSL_MAYBE_UNUSED, RenderState& renderStae)
{
    ...
    params.osl_printf_buffer = d_osl_printf_buffer;
    params.osl_printf_buffer_size = OSL_PRINTF_BUFFER_SIZE;
    ...
    CUDA_CHECK(hipStreamSynchronize(m_cuda_stream));
    ...
    std::vector<char> printf_buffer(OSL_PRINTF_BUFFER_SIZE, 0);
...
OptixGridRenderer::processPrintfBuffer(void* buffer_data, size_t buffer_size)
{
    ...
    while (total_read < buffer_size) {
        ...
        std::cout << "\nRecieved args size: " << args_size << std::endl;
        src += sizeof(size_t);
                ...
                for (size_t i = 0; !format_end_found; i++) {
                    ...
                    case 'e':
                        ...
                        src = (src + sizeof(double) - 1)
                              & ~(sizeof(double) - 1);
                        {
                        const double val = *reinterpret_cast<const double*>(&ptr[src]);
