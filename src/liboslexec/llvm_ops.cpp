...
#    else
#        define OSL_SHADEOP_NOINLINE extern "C" OSL_DEVICE OSL_DLL_EXPORT
#    endif
#endif
...
OSL_SHADEOP void
osl_sincos_fff(float x, void* s_, void* c_)
{
#if OSL_FAST_MATH
...
OSL_SHADEOP void
osl_sincos_dfdff(void* x_, void* s_, void* c_)
{
...
OSL_SHADEOP void
osl_sincos_dffdf(void* x_, void* s_, void* c_)
{
    Dual2<float>& x      = DFLOAT(x_);
    float& sine          = *(float*)s_;
...
OSL_SHADEOP void
osl_sincos_dfdfdf(void* x_, void* s_, void* c_)
{
    Dual2<float>& x      = DFLOAT(x_);
    Dual2<float>& sine   = DFLOAT(s_);
    Dual2<float>& cosine = DFLOAT(c_);
...
OSL_SHADEOP void
osl_sincos_vvv(void* x_, void* s_, void* c_)
{
#if OSL_FAST_MATH
...
OSL_SHADEOP void
osl_sincos_dvdvv(void* x_, void* s_, void* c_)
{
...
#else
    auto sincos_comp = [](const Dual2<float>& x, float& sine_val,
                          float& sine_dx, float& sine_dy, float& cosine_val) {
        float s_f, c_f;
#    if OSL_FAST_MATH
        OIIO::fast_sincos(x.val(), &s_f, &c_f);
#    else
...
OSL_SHADEOP void
osl_sincos_dvvdv(void* x_, void* s_, void* c_)
{
    Dual2<Vec3>& x      = DVEC(x_);
    Vec3& sine          = VEC(s_);
...
#else
    auto sincos_comp = [](const Dual2<float>& x, float& sine_val,
                          float& cosine_val, float& cosine_dx,
                          float& cosine_dy) {
        float s_f, c_f;
#    if OSL_FAST_MATH
        OIIO::fast_sincos(x.val(), &s_f, &c_f);
...
#    endif
    ...
    sincos_comp(comp_x(x), sine.x, cosine.val().x, cosine.dx().x,
                cosine.dy().x);
    sincos_comp(comp_y(x), sine.y, cosine.val().y, cosine.dx().y,
                cosine.dy().y);
    sincos_comp(comp_z(x), sine.z, cosine.val().z, cosine.dx().z,
                cosine.dy().z);
...
OSL_SHADEOP void
osl_sincos_dvdvdv(void* x_, void* s_, void* c_)
{
    Dual2<Vec3>& x      = DVEC(x_);
    Dual2<Vec3>& sine   = DVEC(s_);
    Dual2<Vec3>& cosine = DVEC(c_);
...
#    endif
    ...
    auto sincos_comp = [](const Dual2<float>& x, float& sine_val,
                          float& sine_dx, float& sine_dy, float& cosine_val,
                          float& cosine_dx, float& cosine_dy) {
        float s_f, c_f;
#    if OSL_FAST_MATH
        OIIO::fast_sincos(x.val(), &s_f, &c_f);
#    else
...
#    endif
    ...
    sincos_comp(comp_x(x), sine.val().x, sine.dx().x, sine.dy().x,
                cosine.val().x, cosine.dx().x, cosine.dy().x);
    sincos_comp(comp_y(x), sine.val().y, sine.dx().y, sine.dy().y,
                cosine.val().y, cosine.dx().y, cosine.dy().y);
    sincos_comp(comp_z(x), sine.val().z, sine.dx().z, sine.dy().z,
                cosine.val().z, cosine.dx().z, cosine.dy().z);
...
OSL_SHADEOP void
osl_logb_vv(void* r, void* x_)
{
    const Vec3& x(VEC(x_));
    VEC(r).setValue(OIIO::fast_logb(x.x), OIIO::fast_logb(x.y),
                    OIIO::fast_logb(x.z));
}

OSL_SHADEOP float
osl_floor_ff(float x)
{
    return floorf(x);
}

OSL_SHADEOP void
osl_floor_vv(void* r, void* x_)
{
    const Vec3& x(VEC(x_));
...
OSL_SHADEOP float
osl_round_ff(float x)
{
    return roundf(x);
}

OSL_SHADEOP void
osl_round_vv(void* r, void* x_)
{
    const Vec3& x(VEC(x_));
...
OSL_SHADEOP float
osl_trunc_ff(float x)
{
    return truncf(x);
}

OSL_SHADEOP void
osl_trunc_vv(void* r, void* x_)
{
    const Vec3& x(VEC(x_));
...
OSL_SHADEOP float
osl_step_fff(float edge, float x)
{
    return x < edge ? 0.0f : 1.0f;
}

OSL_SHADEOP void
osl_step_vvv(void* result, void* edge, void* x)
{
...
OSL_HOSTDEVICE inline Dual2<float>
fabsf(const Dual2<float>& x)
{
    return x.val() >= 0 ? x : -x;
}
...
static OSL_HOSTDEVICE OSL_FORCEINLINE float
...
OSL_SHADEOP float
osl_safe_div_fff(float a, float b)
{
    return safe_div(a, b);
}

OSL_SHADEOP int
osl_safe_div_iii(int a, int b)
{
    return (b != 0) ? (a / b) : 0;
}

OSL_SHADEOP float
osl_smoothstep_ffff(float e0, float e1, float x)
{
    return smoothstep(e0, e1, x);
}

OSL_SHADEOP void
osl_smoothstep_dfffdf(void* result, float e0_, float e1_, void* x_)
{
    Dual2<float> e0(e0_);
    Dual2<float> e1(e1_);
    Dual2<float> x = DFLOAT(x_);
...
OSL_SHADEOP void
osl_smoothstep_dffdff(void* result, float e0_, void* e1_, float x_)
{
    Dual2<float> e0(e0_);
...
OSL_SHADEOP void
osl_smoothstep_dffdfdf(void* result, float e0_, void* e1_, void* x_)
{
    Dual2<float> e0(e0_);
    Dual2<float> e1 = DFLOAT(e1_);
    Dual2<float> x  = DFLOAT(x_);
...
OSL_SHADEOP void
osl_smoothstep_dfdfff(void* result, void* e0_, float e1_, float x_)
{
...
OSL_SHADEOP void
osl_smoothstep_dfdffdf(void* result, void* e0_, float e1_, void* x_)
{
    Dual2<float> e0 = DFLOAT(e0_);
    Dual2<float> e1(e1_);
    Dual2<float> x = DFLOAT(x_);
...
OSL_SHADEOP void
osl_smoothstep_dfdfdff(void* result, void* e0_, void* e1_, float x_)
{
    Dual2<float> e0 = DFLOAT(e0_);
    Dual2<float> e1 = DFLOAT(e1_);
    Dual2<float> x(x_);
...
OSL_SHADEOP void
osl_smoothstep_dfdfdfdf(void* result, void* e0_, void* e1_, void* x_)
{
    Dual2<float> e0 = DFLOAT(e0_);
    Dual2<float> e1 = DFLOAT(e1_);
    Dual2<float> x  = DFLOAT(x_);
...
calculatenormal(void* P_, bool flipHandedness)
{
...
OSL_SHADEOP void
osl_filterwidth_vdv(void* out, void* x_)
{
    Dual2<Vec3>& x = DVEC(x_);
