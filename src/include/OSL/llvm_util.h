...
public:
    ...
    llvm::ExecutionEngine* make_jit_execengine(
        std::string* err = nullptr, TargetISA requestedISA = TargetISA::NONE,
        bool debugging_symbols = false, bool profiling_events = false);
    ...
    bool supports_llvm_bit_masks_natively() const
    {
        return m_supports_llvm_bit_masks_natively;
    }
    ...
    /// explicitly instead of relying on dlsym.
    static void add_global_mapping(const char* global_var_name,
    ...
    void validate_struct_data_layout(
        llvm::Type* Ty,
    ...
    struct ScopedMasking {
        ...
        ScopedMasking(ScopedMasking&& other)
            : m_util(other.m_util)
            , m_previous_masking_requirement(
                  other.m_previous_masking_requirement)
        {
    ...
    {
        return m_llvm_type_longlong_ptr;
    }
    ...
    {
        return m_llvm_type_matrix_ptr;
    }
    llvm::PointerType* type_double_ptr() const
    {
        return m_llvm_type_double_ptr;
    }
    ...
    llvm::PointerType* type_wide_char_ptr() const
    {
        return m_llvm_type_wide_char_ptr;
    }
    llvm::PointerType* type_wide_bool_ptr() const
    {
        return m_llvm_type_wide_bool_ptr;
    }
    llvm::PointerType* type_wide_int_ptr() const
    {
        return m_llvm_type_wide_int_ptr;
    }
    llvm::PointerType* type_wide_float_ptr() const
    {
        return m_llvm_type_wide_float_ptr;
    }
    ...
    /// Generate code for a memcpy.
    void op_memcpy(llvm::Value* dst, int dstalign, llvm::Value* src,
...
private:
    ...
    std::vector<MaskInfo>
        m_mask_stack;  // stack for masks that all stores should use when enabled
    ...
    std::vector<MaskedLoopContext>
        m_masked_loop_stack;  // stack to track loop condition & break count

    void set_masking_required(bool required)
    {
        m_is_masking_required = required;
    }
    ...
    llvm::Value* op_combine_8x_vectors(llvm::Value* half_vec_1,
