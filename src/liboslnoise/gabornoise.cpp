...
#endif
        a = Gabor_Frequency
            * ((TWO_to_bandwidth - 1.0) / (TWO_to_bandwidth + 1.0))
            * SQRT_PI_OVER_LN2;
...
gabor_sample(GaborParams& gp, const Vec3& /*x_c*/, fast_rng& rng, Vec3& omega,
             float& phi)
{
    ...
    if (gp.anisotropic == 1 /* anisotropic */) {
        ...
        float sin_omega_p = sqrtf(
            std::max(0.0f, 1.0f - cos_omega_p * cos_omega_p));
...
#endif
        omega = Vec3(cos_omega_t * sin_omega_p, sin_omega_t * sin_omega_p,
                     cos_omega_p)
                    .normalized();
    } else {
        // otherwise hybrid
    ...
    for (int i = 0; i < n_impulses; i++) {
        ...
        if (x_k_i.val().length2() < gp.radius2) {
            if (!gp.do_filter) {
                ...
                // tiny.
                sum += gabor_kernel(gp.weight, omega_i, phi_i, gp.a,
                                    x_k_i);  // 3D
            } else {
                ...
                slice_gabor_kernel_3d(d_i, gp.weight, gp.a, omega_i_t, phi_i,
                ...
                filter_gabor_kernel_2d(gp.filter, w_i_t_s, gp.a, omega_i_t_s,
...
gabor_setup_filter(const Dual2<Vec3>& P, GaborParams& gp)
{
    ...
    Matrix33 Mscreen_to_tex = make_matrix33_cols(P.dx(), P.dy(),
                                                 Vec3(0.0f, 0.0f, 0.0f));
    ...
    Matrix22 Sigma_f_scr(sigma_f_scr * sigma_f_scr, 0.0f, 0.0f,
                         sigma_f_scr * sigma_f_scr);
...
OSL_HOSTDEVICE Dual2<float>
pgabor(const Dual2<float>& x, const Dual2<float>& y, float xperiod,
       float yperiod, const NoiseParams* opt)
{
    // for now, just slice 3D
...
OSL_HOSTDEVICE Dual2<Vec3>
pgabor3(const Dual2<float>& x, const Dual2<float>& y, float xperiod,
        float yperiod, const NoiseParams* opt)
{
    // for now, just slice 3D
